...
void 
MTS_BaseHelper::addSpots (cairo_surface_t *surface, int num_min, int num_max,
                          double size_min,double size_max, double diminish_rate,
                          bool transparent, int color_min, int color_max){
    ...
    // iterate through mask and set pixels in range of hole to have value 255.
    ...
    for (int i = 0; i < num_spots; i++) {
        ...
        for (int row = 0; row < height; row++) {
            for (int column = 0; column < stride; column++) {
                double dis = pow(pow((double)(row-y),2) +
                                 pow((double)(column-x),2),0.5);
...
MTS_BaseHelper::points_to_path(cairo_t *cr, vector<coords> points,
    ...
    cairo_curve_to(cr, cp1.first, cp1.second, cp2.first, cp2.second,
    ...
    while (count > 0) {
        ...
        if (x == u) {
            ...
            /*solve 
               ...
               c = (y-w-first_degree_derivative*(x-u)-d*((pow(x,3)-pow(u,3))
                    -3*pow(x,3)*(x-u)))/(pow(x,2)-pow(u,2)-2*x*(x-u));
            ...
            d = (y-w-(y-m)*k-j*(y-w-first_degree_derivative*(x-u)))/
              (x_3-u_3-(x_3-n_3)*k-j*(x_3-u_3-3*x_2*(x-u)));
        ...
        cairo_curve_to(cr, cp1.first, cp1.second, cp2.first,
                       cp2.second, end.first, end.second);
...
MTS_BaseHelper::make_points_wave(double length, double height, 
    ...
    for(int i = num_points - 1; i >= 0; i--) {
        ...
        //ensure points stay above the bottom of the canvas
        y = height - y_variance;
